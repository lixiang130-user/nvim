...
-- 复制当前光标单词到寄存器z,在搜索\<xxxx\>b当前单词的第一个字母
-- :let @@ = @0   复制寄存器0到默认寄存器
map("n", "gd", "\"zyiwb/\\<<C-r>z\\><cr>:let @@ = @0<CR>", opt)
-- v 模式下 * 和 # 可以直接匹配选中的内容
...
map('n', 'co', ':lua CloseBashBuffersAndCloseOthers()<CR>:NvimTreeToggle<CR><C-w>l', opt)    --<leader>co + cb
